    ...
    return `
        <div class="relative" id="dropdown-${id}">
            <button type="button"
                ...
                <i data-lucide="chevron-down" class="w-4 h-4 text-fg3 transition-transform"></i>
            </button>
            <ul class="absolute top-full left-0 mt-1 min-w-full w-max border border-border rounded-xs bg-bg1 shadow-md z-10 overflow-hidden py-1 hidden"
                data-dropdown-menu="${id}">
                <li class="flex items-center justify-between px-3 py-2 text-body-sm cursor-pointer hover:bg-bg2 text-fg4"
                    data-dropdown-value="" data-dropdown-id="${id}">
                    ${placeholder}
                </li>
                ${options.map(option => `
...
export function bindDropdown(
    ...
    const trigger = document.querySelector<HTMLButtonElement>(`[data-dropdown-trigger="${id}"]`);
    const menu = document.querySelector<HTMLUListElement>(`[data-dropdown-menu="${id}"]`);
    ...
    function updateLabel(selectedValue: string) {
        ...
        if (option) {
            const selectedItem = menu.querySelector(`[data-dropdown-value="${selectedValue}"]`);
            if (selectedItem) {
                selectedItem.insertAdjacentHTML("beforeend", '<i data-lucide="check" class="w-4 h-4 text-fg1"></i>');
            }
        }
    }
    ...
    trigger.addEventListener("click", (e) => {
        ...
        document.querySelectorAll("[data-lucide='chevron-down']").forEach(icon => {
            (icon as HTMLElement).style.transform = "";
        });

        if (!isOpen) {
            menu.classList.remove("hidden");
            (trigger.querySelector("[data-lucide='chevron-down']") as HTMLElement).style.transform = "rotate(180deg)";
        }
    });
    ...
    menu.querySelectorAll("[data-dropdown-value]").forEach(item => {
        item.addEventListener("click", () => {
            ...
            (trigger.querySelector("[data-lucide='chevron-down']") as HTMLElement).style.transform = "";
        });
    });
